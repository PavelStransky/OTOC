...
const int numT = 256;									// Number of time steps
const double startT = 1E6;								// First time
const double stepT = 20000;  							// Size of the time step
...
#define RESULT "./j50motocs/OTOC_f_2._j_50_nmax_300_k_%d_T_1E6_20000_256_pq.dat"
...
void *Thread(void *data) {
	...
	for(int ti = td->i; ti < numT; ti += threads) {
		...
		printf("%d: Time %0.0lf (%d) in thread %d calculated in %lds. OTOC = %lf\n", td->n, t, ti, td->i, time(0) - start, result[ti]);
	}
